...
/**
 * Service implementation for managing users (Teachers, Students, Admins).
 */
@Service
public class UserService implements IUser<UserDto, String> {
  ...
  /**
   * Creates a new user based on the provided user type.
   *
  ...
  /**
   * Deletes a user and handles associated data (enrollments for students, courses for teachers).
   *
   * @param s the ID of the user to delete
   */
  @Override
  @Transactional
  public void deleteUser(String s) {
    ...
    if (user != null) {
      if (user instanceof Student student) {
        ...
        List<Enrollment> enrollmentsByUuid = enrollmentRepository
            .findByStudent_StudentIdAndStatus(student.getId(), EnrollmentStatus.ENROLLED);
        enrollmentRepository.deleteAll(enrollmentsByUuid);
      } else if (user instanceof Teacher teacher) {
        // Nullify teacher in course groups
        List<CourseGroup> groups = courseGroupRepository.findAllByTeacher(teacher);
